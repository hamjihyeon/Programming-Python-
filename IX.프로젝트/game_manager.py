...
class GameManager_GUI:
    def __init__(self):
        self.ttt = TicTacToe()
        CAMBUS_SIZE = 300
        self.TILE_SIZE = CAMBUS_SIZE/3

        self.root = tkinter.Tk()
        self.root.title("틱 택 토")
        self.root.geometry(str(CAMBUS_SIZE)+"x"+str(CAMBUS_SIZE)) # geometry("300x300")
        self.root.resizable(width = False, height = False)
        self.canvas = tkinter.Canvas(self.root, bg = "white", width = CAMBUS_SIZE, height = CAMBUS_SIZE)
